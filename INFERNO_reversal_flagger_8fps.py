...
########commented block start########
#The angle of the vector (from point i-4 to point i) was calculated using the 2-argument arctangent function
            if (x_4frame_average[i]-x_4frame_average[i-4]==0) and (y_4frame_average[i]<=y_4frame_average[i-4]):
                angle_vector.append(-math.pi/2)
            else:
                angle_vector.append(math.atan2(y_4frame_average[i]-y_4frame_average[i-4],x_4frame_average[i]-x_4frame_average[i-4]))
########commented block end########
...
########commented block start########
# For each point of the trajectory (corresponding to frame i), the angle change was calculated between the angles of two vectors: 
# (a) the vector from coordinates at frame i-4  to coordinates at frame i 
# (b) the vector from coordinates at frames i  to coordinates at frame i+4 
            if (math.fabs(angle_vector[i]-angle_vector[i+4])>math.pi):
                angle_change.append(math.fabs(angle_vector[i]-angle_vector[i+4])-2*math.pi)
            else:
                angle_change.append(angle_vector[i]-angle_vector[i+4])
########commented block end########
